...
func TestUpdateEvent(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("some error when get event %s\n", err)
	}

	if !isEventEquals(event2, event, false) {
...
func TestGetEvents(t *testing.T) {
	...
	allEvents2, _ := calendar.GetEventsByTimestampsPeriod(nil, nil)
	if len(allEvents2) != 7 {
		t.Error("7 events must be in calendar and GetEventsByTimestampsPeriod(nil, nil) must return all of them")
	}
	...
	eventTime := ts(2019, 11, 18, 8, 0)
	eventList, _ := calendar.GetEventsByTimestampsPeriod(nil, eventTime)
	...
	eventTime = ts(2019, 11, 24, 8, 0)
	eventList, _ = calendar.GetEventsByTimestampsPeriod(eventTime, nil)
	...
	startEventTime := ts(2019, 11, 20, 8, 0)
	endEventTime := ts(2019, 11, 22, 8, 0)
	eventList, _ = calendar.GetEventsByTimestampsPeriod(startEventTime, endEventTime)
	...
	startEventTime = ts(2019, 11, 20, 8, 1)
	endEventTime = ts(2019, 11, 20, 9, 59)
	eventList, _ = calendar.GetEventsByTimestampsPeriod(startEventTime, endEventTime)
...
func addEvent(t *testing.T, calendar *Calendar, event *Event, expectedCount int) int {
	...
	if err != nil {
		...
		t.Errorf("calendar calendar must has %d events instead of %d\n", expectedCount, calendar.getEventsTotalCount())
		return -3
	}
...
func addFixedListOfEvents(t *testing.T, calendar *Calendar) {
	addEvent(t, calendar, &Event{
		Name:  "Monday",
		Start: ts(2019, 11, 18, 8, 0),
		End:   ts(2019, 11, 18, 10, 0),
	}, 1)

	addEvent(t, calendar, &Event{
		Name:  "Tuesday",
		Start: ts(2019, 11, 19, 8, 0),
		End:   ts(2019, 11, 19, 10, 0),
	}, 2)

	addEvent(t, calendar, &Event{
		Name:  "Wednesday",
		Start: ts(2019, 11, 20, 8, 0),
		End:   ts(2019, 11, 20, 10, 0),
	}, 3)

	addEvent(t, calendar, &Event{
		Name:  "Thursday",
		Start: ts(2019, 11, 21, 8, 0),
		End:   ts(2019, 11, 21, 10, 0),
	}, 4)

	addEvent(t, calendar, &Event{
		Name:  "Friday",
		Start: ts(2019, 11, 22, 8, 0),
		End:   ts(2019, 11, 22, 10, 0),
	}, 5)

	addEvent(t, calendar, &Event{
		Name:  "Saturday",
		Start: ts(2019, 11, 23, 8, 0),
		End:   ts(2019, 11, 23, 10, 0),
	}, 6)

	addEvent(t, calendar, &Event{
		Name:  "Sunday",
		Start: ts(2019, 11, 24, 8, 0),
		End:   ts(2019, 11, 24, 10, 0),
	}, 7)
