...
// Inner helper that convert grpc timestamp.Timestamp to calendar.EventTime
func convertToCalendarEventTime(tspb *timestamp.Timestamp) (*calendar.EventTime, error) {
...
// Inner Helper that helps convert grpc.Event to calendar.Event
func convertToCalendarEvent(event *Event) (*calendar.Event, error) {
	...
	calendarEvent := calendar.NewEventWithId(int(event.Id), event.Name, *startTime, *endTime)

	return &calendarEvent, nil
}

// Convert from inner Event entity (calendar.Event) to grpc.Event
func convertFromCalendarEvent(calendarEvent calendar.Event) (*Event, error) {
